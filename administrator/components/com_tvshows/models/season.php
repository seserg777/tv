...
class TvshowsModelSeason extends JModelAdmin
{
	...
	public function delete(&$pks){
		...
		foreach ($pks as $i => $pk){
			if ($table->load($pk)){
				//echo '<pre>';var_dump($table);exit;
				$query = $db->getQuery(true);
				$conditions = array(
					$db->quoteName('tv_id') . ' = ' . $db->quote($table->film), 
					$db->quoteName('season_number') . ' = ' . $db->quote($table->season_number)
				);
				$query->delete($db->quoteName('#__tvshows_episodes'));
