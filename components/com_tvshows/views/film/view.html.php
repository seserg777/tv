...
/**
 * Film item view class.
 *
 * @package     Tvshows
 * @subpackage  Views
 */
class TvshowsViewFilm extends JViewLegacy
{
	protected $item;
	protected $form;
	protected $state;
	...
	public function display($tpl = null)
	{
		...
		if (empty($this->item))
		{
			$app->redirect(JRoute::_('index.php?option=com_tvshows&view=films'), JText::_('JERROR_NO_ITEMS_SELECTED'));
		}
		...
		$this->item->tags->getItemTags('com_tvshows.film' , $this->item->id);
		...
		for($i = 1; $i<=(count($this->item->seasons)+1);$i++){
	...
	protected function _prepareDocument()
	{
		...
		$film_meta_title_pattern = $this->component_params->get('film_meta_title_pattern', null);
		$film_meta_description_pattern = $this->component_params->get('film_meta_description_pattern', null);
		...
		if ($menu && ($menu->query['option'] !== 'com_content' || $menu->query['view'] !== 'article' || $id != $this->item->id))
		{
			...
			{
				if(stristr($item['link'], 'view=category') != false){
		...
		if(isset($film_meta_title_pattern) && !empty($film_meta_title_pattern)){
			$custom_title = str_replace('{seasons_enumeration}', $this->item->count_seasons, str_replace('{film_name}', $this->item->title, $film_meta_title_pattern));
			
			if(count($this->item->implodetags)){
				$custom_title = str_replace('{film_genres}', $this->item->implodetags, $custom_title);
			}
			
			if(isset($this->item->creators) && !empty($this->item->creators)){
				$custom_title = str_replace('{film_creators}', $this->item->creators, $custom_title);
			}
			
			if(isset($this->item->channel) && !empty($this->item->channel)){
				$custom_title = str_replace('{film_channel}', $this->item->channel, $custom_title);
			}		
		...
		if(isset($film_meta_description_pattern) && !empty($film_meta_description_pattern)){
			$custom_description = str_replace('{seasons_enumeration}', $this->item->count_seasons, str_replace('{film_name}', $this->item->title, $film_meta_description_pattern));
			...
			if(isset($this->item->creators) && !empty($this->item->creators)){
				$custom_description = str_replace('{film_creators}', $this->item->creators, $custom_description);
			}
			
			if(isset($this->item->channel) && !empty($this->item->channel)){
				$custom_description = str_replace('{film_channel}', $this->item->channel, $custom_description);
			}
